...
class Thinkchat {
	...
	public function message($from, $to, $body, $action='batchusers', $device=''){
		...
		if( $this->currCheckNums >= $this->checkNums ) {
			$this->currCheckNums = $this->checkNums == 0 ? 1 : ($this->currCheckNums++) % ($this->checkNums);
			$result = $this->lincense->checkAuthOnline(self::useronlinenums());
			if($result && $result['code'] != 0)
					return showData(null, $result['msg'], 1);
		}
		// 发送消息
	 ...
	 * @param unknown $other
	 */
	public function notice($from, $to, $type=1, $content='',$other='', $device='', $extend=''){
		...
		$notice = array(
		        ...
		        'extend' => $extend ? $extend : new \stdClass(),
				'time'	 => getMillisecond()
		);
