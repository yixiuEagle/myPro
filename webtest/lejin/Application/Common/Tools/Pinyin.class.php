 ...
 * 类用法
 * $pinyin = new Pinyin(); 
 * $pinyin->Pinyin('这是WEB开发网'); //默认是gb编码
 * $pinyin->Pinyin('这是WEB开发网',1); //第二个参数随意设置即为utf8编码
 */
namespace Common\Tools;
class Pinyin {
	function __construct(){}
	static function pinyin($_String, $_Code='gb2312', $_Num=null){
		...
		if ( is_numeric(substr($_Res, 0, 1)) ) return '';
		$chars = preg_replace ( "/[^a-z]*/", '',  $_Res);
		if ($_Num) {
