...
<table id="userList_<?php echo ($rand); ?>" class="easyui-datagrid" title="<?php echo ($currentpos); ?>" data-options="
	...
	url:'<?php echo U('User/ex_user_list',array('uid'=>I('uid',0)));?>',idField:'uid',border:false,
...
function userOperateText(val, row, index){
	var btn = [];
		btn.push('<a href="javascript:void(0);" onclick="goAccount_<?php echo ($rand); ?>('+row.uid+')">消费明细</a>');
	return btn.join(' | ');
}
//搜索用户
...
function goAccount_<?php echo ($rand); ?>(id) {
	var url = "<?php echo U('User/account_log');?>";
	url += "?uid="+id;
	if($('#pagetabs').tabs('exists', "消费明细")){
		$('#pagetabs').tabs('close', "消费明细");
		$('#pagetabs').tabs('add',{
			title: "消费明细",
			href: url,
			closable: true,
