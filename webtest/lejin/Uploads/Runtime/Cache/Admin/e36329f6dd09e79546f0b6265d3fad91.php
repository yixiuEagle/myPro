...
$(function(){
	...
	$.formValidator.initConfig({formID:"AdminEditInfoForm",theme:'App',onSuccess:AdminInfoSave,inIframe:true});
...
function AdminInfoSave(){
	...
	<form id="AdminEditInfoForm" style="font-size: 13px">
		<table cellspacing="10" style="border: 1px solid #ccc; width: 500px;">
			...
			<tr>
				<td>最后登录时间</td>
				<td><?php if($info["lastlogintime"] > 0): echo (date('Y-m-d
					H:i:s',$info["lastlogintime"])); else: ?>-<?php endif; ?></td>
				<td></td>
			</tr>
			<tr>
				<td>最后登录IP</td>
			...
			<tr>
				<td>真实姓名</td>
				<td width="187">
					<input type="text" name="info[realname]" id="realname" value="<?php echo ($info["realname"]); ?>" style="width: 180px; height: 22px" /></td>
				<td><div id="realnameTip"></div></td>
			</tr>
			<tr>
				<td>E-mail：</td>
				<td>
					<input type="text" name="info[email]" id="email" value="<?php echo ($info["email"]); ?>" style="width: 180px; height: 22px" /></td>
				<td><div id="emailTip"></div></td>
			</tr>
			<tr>
				<td></td>
				<td colspan="2" align="left">
					<a id="AdminInfoSubbtn" class="easyui-linkbutton" style="width: 50px;">提交</a>
				</td>
			</tr>
		</table>
	</form>
