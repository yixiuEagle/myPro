...
class SessionModel extends Model {
	...
	public function _list($map,$uid){
	    ...
	    $total = M('session_user')->alias('su')->where($map)->count();
	    if ($total){
	        $page  = page($total);
	...
	function public_list($uid, $map, $limit, $order='createtime desc'){		
		...
		//$list  	  = $this->alias('s')->field($field)->where($map)->join($join)->limit($limit)->select();
	...
	private function sendNotice($uid, $id, $type, $fid=0){
		...
		switch ($type) {
			case 300:
				$to['touid'] = self::noticeToUsers($id,$uid);
			    //$uidsArr  = M($this->sessionUser)->field('uid')->where(array('sessionid'=>$id,'uid'=>array('neq',$uid)))->select();
			    //$to['touid'] = $v['uid'];
			    $thinkchat->notice($userinfo, $to, $type, $userinfo['nickname'].'退出了该会话('.$room['name'].')', $room);
				break;
			case 301:
				$to['touid'] = $fid;
				$thinkchat->notice($userinfo, $to, $type, '你被管理员踢出了该会话('.$room['name'].')', $room);
				break;
			case 302:
				$to['touid'] = self::noticeToUsers($id,$uid);
				$thinkchat->notice($userinfo, $to, $type, '管理员'.$userinfo['nickname'].'修改了会话名称为:'.$room['name'], $room);
				break;
			case 303:
				$to['touid'] = self::noticeToUsers($id,$uid);
				$thinkchat->notice($userinfo, $to, $type, '管理员'.$userinfo['nickname'].'解散了该会话('.$room['name'].')', $room);
				break;
			case 304:
				$to['touid'] = self::noticeToUsers($id,$uid);
				$fuserinfo   = $user->getUserName($fid);
				$thinkchat->notice($fuserinfo, $to, $type, $fuserinfo['nickname'].'被管理员踢出了该会话('.$room['name'].')', $room);
				break;
		}
	}
	/**
