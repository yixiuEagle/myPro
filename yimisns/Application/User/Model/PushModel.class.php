...
// 		Log::write("handleIosPush from=" . $newmsg);
		return $newmsg;
	}
	/**
	 * 取出json消息体
	...
	private function iosmsg($devToken, $msg, $badge){
		...
		if (!$fp) return false;
	   $uid=M('user_push')->field('uid')->where(array('udid'=>$devToken))->find();
	   $msg=M('user')->field('msg_setting,wurao_start,wurao_end')->where(array('uid'=>$uid['uid']))->find();
	   $start=explode(":",$msg['wurao_start'])[0];
	   $end=explode(":",$msg['wurao_end'])[0];
	   if(date('H',time())>=$start && date('H',time())<=$end){
	       $iswurao='1'; 
	   }else{
	       $iswurao='0';   
	   }
	   if(substr($msg['msg_setting'],0,1)=='1' && $iswurao=='0'){
	       $sound='default';
	   }else{
	       $sound='';
	   }
...
// 		Log::write("list=".json_encode($list));
...
// 			Log::write("handleIosPush newData=" . json_encode($newData));
			$fromID = self::parseFromFiled($str);
			//系统通知
			/* if(strpos($fromID, "admin") == 0 || strpos($fromID, "beautyas") == 0 ){
				$msg = '';
			} */
			...
			if ($msg){
// 				Log::write("handleIosPush send msg=" . $msg);
