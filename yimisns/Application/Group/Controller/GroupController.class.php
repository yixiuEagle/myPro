	...
	function index($page=1, $rows=10, $search=array()){
		if (IS_POST) {
			...
			foreach ($list as $key=>$value){
			    $data=M('user')->where(array('uid'=>$value['uid']))->find();
			    $list[$key]['creator'] =$data['nickname'];
			    if($value['type']==0){
			        $list[$key]['type1']='快拍群';
			    }else if($value['type']==2){
			        $list[$key]['type1']='位置共享群';
			    }
			    //$list[$key]['logosmall']=makeHttpHead($value['logosmall']);
				//格式化时间
...
// 			$this->info = $this->group_db->public_list(0, array('id'=>$id), 1);
			$data=M('group')->where(array('id'=>$id))->find();
		    $user=M('user')->where(array('uid'=>$data['uid']))->find();
			$info=array(
			    'id'=>$data['id'],
			   // 'logosmall'=>makeHttpHead($data['logosmall']),
			    'creator'=>$user['nickname'],
			    'name'=>$data['name'],
			    'description'=>$data['description'],
			    'createtime'=>$data['createtime']
			);
	       M('group')->where(array('id'=>$id))->save(array('description'=>$data['description']));
