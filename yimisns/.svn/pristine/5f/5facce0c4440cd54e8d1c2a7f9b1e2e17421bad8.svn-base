...
class InstallController extends Controller {
	protected function _initialize(){
		...
		if(file_exists('/Application/Install/data/install.lock')){
			$this->error('已经成功安装了ThinkChat，请不要重复安装!');
		}
	}
	//安装第一步，检测运行所需的环境设置
	...
	public function step2($db = null, $admin = null){
		if(IS_POST){
			...
			//检测消息服务器地址
			$imip = $_POST['imserverip'];
 			if(empty($imip))
 				$this->error('请填写消息服务器IP或域名');
			//检测数据库配置
			if(!is_array($db) || empty($db[0]) ||  empty($db[1]) || empty($db[2]) || empty($db[3])){
				...
				list($DB['DB_TYPE'], $DB['DB_HOST'], $DB['DB_NAME'], $DB['DB_USER'], $DB['DB_PWD'],
						$DB['DB_PORT'], $DB['DB_PREFIX'], $DB['imserverip']) = $db;
				//缓存数据库配置
			...
			session('error') && $this->error('环境检测没有通过，请调整环境后重试！' . session('errorinfo') );
			$step = session('step');
			if($step != 1 && $step != 2){
				$this->redirect('step1');
			}
