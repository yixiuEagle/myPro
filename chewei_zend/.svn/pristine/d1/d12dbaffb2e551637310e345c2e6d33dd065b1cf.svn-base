 ...
 * @param String $languageCode -- 多语言的编码
 * @param Array $languageParams -- 多语言中的文字需要带参数时候，替换相应位置的字符内容。
 * @return array
 */
function showData($data, $msg='', $code=0, $page='', $debugMsg = '', $languageCode = '', $languageParams=array() ){
	if($languageCode == '') {
		return array(
				'data'		 	=> $data,
				'msg'  			=> $msg,
	...
	$newMsg = C($languageCode);
	if($languageParams) {
		$index = 0;
		foreach ($languageParams as $value) {
			$replace = "{" . $index . "}";
			$newMsg = str_replace($replace, $value, $newMsg);
			$index += 1;
		}
	}
	
	if (C('LOG_OUTPUT_RESULT')){// 需要输出结果
		$output = array(
			'data'		 	=> $data,
			'msg'  			=> $newMsg,
			'debugMsg'  	=> $debugMsg,
		...
		Log::write("output result=" . json_encode($output));
	}
...
//     $thumbHeight = 320;
    ...
    $info   =   $upload->upload();
    Log::write('shangchuanwanbi='.date('Y-d-m H-i-s',time()));
    if(!$info) {
...
function upload($savePath, $uid, $type){
	...
	$upload 	= new Upload();
	$upload->exts 	   = array('jpg','png','gif','txt');// 设置附件上传类型
