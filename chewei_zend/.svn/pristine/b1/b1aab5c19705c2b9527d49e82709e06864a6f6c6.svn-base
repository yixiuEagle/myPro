...
class LogsController extends CommonController {
    ...
    function website(){
    	if (IS_POST){
    		 C('LOG_REQUEST_ARGUMENT', 2);
    		 C('LOG_OUTPUT_RESULT', $info['LOG_OUTPUT_RESULT']);
    		 C('APP_CHECK_URL_SAFE', $info['APP_CHECK_URL_SAFE']);
    		 C('FRAMEWORK_VERSION', $info['FRAMEWORK_VERSION']);
    		 C('WEBSITE_NAME', $info['WEBSITE_NAME']);
    		 C('WEBSITE_KEYWORD', $info['WEBSITE_KEYWORD']);
    		
    		$this->success($info['LOG_REQUEST_ARGUMENT']);
    		//$this->success('修改成功');
    	}else {
    		$info['LOG_REQUEST_ARGUMENT'] = C('LOG_REQUEST_ARGUMENT');
    		$info['LOG_OUTPUT_RESULT'] = C('LOG_OUTPUT_RESULT');
    		$info['APP_CHECK_URL_SAFE'] = C('APP_CHECK_URL_SAFE');
    		$info['FRAMEWORK_VERSION'] = C('FRAMEWORK_VERSION');
    		$info['WEBSITE_NAME'] = C('WEBSITE_NAME');
    		$info['WEBSITE_KEYWORD'] = C('WEBSITE_KEYWORD');
