<?php
if (!defined('CODETABLEDIR')) define('CODETABLEDIR', COMMON_PATH.'Data/Encoding/');
if(!IS_BAE){	//BAE已存在相同的函数
	/**
	 * utf8转gbk
	 * @param $utfstr
	 */
 ...
 * @param $txt
 */
function gbk_to_pinyin($txt,$current_charset='utf-8') {
	if(strtolower($current_charset) != 'gbk') {
		$txt = iconv($current_charset,'GBK',$txt);
	}	
	$l = strlen($txt);
	$i = 0;
	$pyarr = array();
